...
The raking spike adds persistent raster-space displacement arrays, `rakeDx[]` and `rakeDy[]`, plus a `rakeCut[]` trough mask parallel to the field grid. The top toolbar's `rake` mode, Shift-drag, or two-finger drag writes into these buffers. The stroke is applied as a smoothed/resampled 4-tooth comb, so one drag carves negative-space channels through the pigment and also pulls nearby interior material instead of depending on sparse browser pointer events. The next field solve samples each metaball pool at the displaced coordinates and suppresses field strength inside the comb teeth, giving a first-pass "metaball turns into raster-deformable substrate" behavior without changing lobe geometry. Border cells and owner seams receive weaker displacement force, but the cut mask itself can punch through to paper so the rake responds as deep as the pointer travels.
