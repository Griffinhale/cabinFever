...
The raking spike adds two persistent raster-space displacement arrays, `rakeDx[]` and `rakeDy[]`, parallel to the field grid. The top toolbar's `rake` mode, Shift-drag, or two-finger drag writes offsets into interior cells near the stroke. The stroke is applied as a smoothed/resampled 4-tooth comb, so one drag deforms the same metaball in several parallel channels at once instead of depending on sparse browser pointer events. The next field solve samples each metaball pool at the displaced coordinates, giving a first-pass "metaball turns into raster-deformable substrate" behavior without changing lobe geometry. Border cells and owner seams receive much weaker rake force so silhouettes remain more recognizable.
